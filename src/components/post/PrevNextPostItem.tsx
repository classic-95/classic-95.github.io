...
import { ellipsis } from "../../libs/styles/utils";
...
const Circle = styled.div`
	...
	border: 1px solid ${palette.viloet[5]};
	font-size: 1.5rem;
...
const Container = styled(Link)<{ $right: boolean }>`
	...
	border-bottom: 2px solid ${palette.viloet[2]};
	width: 100%;
	height: 4rem;
	display: flex;
	align-items: center;
...
interface Props {
	right: boolean;
	post: ContinuedPost | null;
	lang: LanguageType;
}

export default function PrevNextPostItem({ right, post, lang }: Props) {
	if (!post) {
		return null;
	}
	return (
		<Container $right={right} to={post.fields.slug}>
